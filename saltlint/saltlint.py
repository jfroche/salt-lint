...
def tabcheck(content, filename):
    ...
    for index, line in enumerate(content.split("\n")):
        if prog.search(line):
            ERRORS.append(ParseError(filename, index + 1,
                                     'Tab character(s) found'))
    return ERRORS
    ...
    for index, line in enumerate(content.split("\n")):
        ...
        if current > previous:
            ...
            if nested == 1:
                if tabsize != 4:
                    ERRORS.append(ParseError(filename, index + 1,
                                             'Four space soft tabs for nested dict not found - %s space tab found' % tabsize))
            elif tabsize not in [0, required]:
                ERRORS.append(ParseError(filename, index + 1,
                                         'Two space soft tabs not found - %s space tab found' % tabsize))
...
def run(filename):
    ...
    if len(ERRORS) == 0:
        ERRORS = ERRORS + trailingwscheck(content, filename) + \
            indentcheck(content, filename) + slscheck(content, filename)
    return ERRORS
